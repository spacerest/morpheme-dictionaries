...
WORD_LISTS_DIR = Path(__file__).parent.parent / "word-lists"
...
LANGUAGE_NOTES = {
  ...
  - Write in Cyrillic script""",
    ...
    "sl": """\
  - Include words with productive verbal prefixes that clearly change meaning:
    po- (after/completive), pri- (arrival/addition), za- (begin/behind/completive),
    pre- (over/through/re-), raz- (apart/spread), iz- (out/from), od- (away from),
    nad- (above/over), pod- (under), so-/su- (co-/together), brez- (without)
  - Include words with common derivational suffixes: -ost (abstract noun: -ness/-ity),
    -anje/-enje (verbal noun: -ing/-tion), -ec/-ka (agent noun: -er), -ni/-ski (adjective)
  - Include compound words and prefixed nouns where the structure is transparent:
    rokopis (roka+pisati = hand+write), brezdomec (brez+dom = without+home),
    samozavest (samo+zavest = self+consciousness), sovrstnik (so+vrsta = co+peer)
  - Good candidates: predlagati, razumevanje, sporazumevanje, izobraževanje, brezposelnost,
    samospoštovanje, popotnik, dopisnik, priseljenec, podpisati, nadgraditi, razdeliti
  - AVOID simple monomorphemic vocabulary with no decomposable structure:
    voda (water), zrak (air), zemlja (earth), hrana (food), glasba (music),
    moč (power), sreča (happiness), svoboda (freedom), ljubezen (love), resnica (truth)
...
EN_LANGUAGE_NOTES = {
  ...
  - Mix nouns, verbs (infinitives), adjectives""",
    ...
    "sl": """\
  - Choose English words whose Slovenian translation is a native Slavic word — NOT an English loanword
  - AVOID words already borrowed into Slovenian: computer/kompjuter, internet/internet, hotel/hotel, sport/šport, bank/banka, film/film, bus/avtobus, taxi/taksi
  - Focus on words with clear Latin/Greek or Germanic morphemes where the Slovenian equivalent is a native derived or compound word
  - Ideal: words where the English morpheme breakdown (prefix+root+suffix) parallels transparent Slovenian structure — double insight for the learner
  - Good candidates: independence (neodvisnost), understanding (razumevanje), impossible (nemogoč), friendship (prijateljstvo), transportation (prevoz), responsibility (odgovornost), knowledge (znanje), freedom (svoboda), leadership (vodstvo), development (razvoj)
  - Include words with productive affixes: un-/in-/im-, re-, pre-, inter-, over-, under-, -tion/-sion, -ness, -ful, -less, -able/-ible, -er/-or, -ment
  - Mix nouns, verbs (infinitives), adjectives""",
...
def main():
    ...
    for code, language in targets.items():
        ...
        tracker = CostTracker(script="generate_wordlists", pair=pair, model=args.model)
