...
from morpheme_db import get_db, split_pair, get_done_ids, insert_entry, get_known_issues_text, mark_word_done, get_canonical_labels, format_canonical_labels_for_prompt
...
def fill_template(template: str, home: str, target: str, words: str = "", canonical_labels: str = "") -> str:
    return (
        template
        .replace("<HOMELANG>", home)
        .replace("<TARGETLANG>", target)
        .replace("<WORDS>", words)
        .replace("<CANONICAL_LABELS>", canonical_labels)
    )
...
def parse_response(text: str) -> list:
    ...
    if text.startswith("```"):
        ...
        if text.endswith("```"):
            text = text.rsplit("\n```", 1)[0].strip()
    # Normalize fullwidth JSON punctuation that Claude sometimes emits for CJK home langs
    ...
    text = text.replace("\u300c", '"')   # 「 left corner bracket
    text = text.replace("\u300d", '"')   # 」 right corner bracket
...
def main():
    ...
    # Load canonical labels from DB for prompt injection
    ...
    system = fill_template(load_prompt("system.txt", pair), args.home, args.target, canonical_labels=canonical_labels)
    ...
    tracker = CostTracker(script="generate_claude", pair=pair, model=args.model)

    if args.preflight:
        from preflight import run_preflight
        run_preflight(
            client, mode="generate", system_prompt=system,
            target_lang=target_lang, home_lang=home_lang,
            glossary_count=len(glossary_details),
            canonical_labels=canonical_labels,
            sample_items=remaining[:5],
