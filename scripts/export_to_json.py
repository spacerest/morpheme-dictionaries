...
LANG_CODES = {
    ...
    "korean": "ko", "mandarin": "zh", "norwegian": "no", "polish": "pl",
    "portuguese": "pt", "russian": "ru", "slovenian": "sl", "spanish": "es",
    "swahili": "sw", "swedish": "sv", "turkish": "tr",
}


def resolve_lang(code_or_name: str) -> str:
    """Accept ISO code or full language name (case-insensitive), return ISO code.
    Unknown values are passed through unchanged (preserves custom codes like 'SPELLING')."""
    return LANG_CODES.get(code_or_name.lower(), code_or_name)
...
def export_pair(conn, target_lang: str, home_lang: str, output_dir: Path, to_verify: bool = False, word_set: str = None, audited_after: str = None) -> int:
    ...
    entries = get_entries(conn, target_lang, home_lang, to_verify=to_verify, word_set=word_set, audited_after=audited_after)
    if not entries:
...
def main():
    ...
    parser.add_argument(
        "--to-verify",
        action="store_true",
        help="Export only to_verify=1 entries",
    )
    parser.add_argument(
        "--word-set",
        metavar="NAME",
        default=None,
        help="Export entries tagged with this word_set value (e.g. 'first_release_dictionary')",
    )
    parser.add_argument(
        "--audited-after",
        metavar="DATE",
        default=None,
        help="Only export entries audited on or after this date (ISO format: YYYY-MM-DD)",
    )
    ...
    if args.target_lang:
        args.target_lang = resolve_lang(args.target_lang)
    if args.home_lang:
        args.home_lang = resolve_lang(args.home_lang)
    ...
    if args.all:
        ...
        for target_lang, home_lang in pairs:
            total += export_pair(conn, target_lang, home_lang, output_dir, to_verify=args.to_verify, word_set=args.word_set, audited_after=args.audited_after)
        print(f"\nDone. {total} total entries exported.")
    else:
        # Single pair mode
        total = export_pair(conn, args.target_lang, args.home_lang, output_dir, to_verify=args.to_verify, word_set=args.word_set, audited_after=args.audited_after)
        if total == 0:
            sys.exit(1)
