#!/usr/bin/env python3
"""
Fix flagged morpheme dictionary entries using a Sonnet pass.
...
LANG_FROM_CODE = {
    ...
    "sv": "Swedish", "tr": "Turkish", "sl": "Slovenian",
}
...
def run_fixes(
    ...
    """Run the fix API pass. Returns {word_id: fixed_entry_dict}."""
            ...
            try:
                ...
                for fix in fixes:
                    ...
                    if wid and wid in entries_by_id:
                        fixed[wid] = merge_fix(entries_by_id[wid], fix)
                print(f"done ({len(fixed)}/{len(flagged_ids)})")
                break
...
def main():
    ...
    if args.target_lang:
        ...
        fixed = run_fixes(flagged_ids, entries_by_id, flags_by_id, home_lang_name, client, args.model, args.batch_size)

        for wid, fixed_entry in fixed.items():
            update_entry(conn, target_lang, home_lang, wid, fixed_entry)
            for flag_id in flag_ids_by_word.get(wid, []):
                resolve_flag(conn, flag_id, status="fixed")

        print(f"\nDone. {len(fixed)} entries fixed in DB for [{target_lang}-{home_lang}]")
        conn.close()
